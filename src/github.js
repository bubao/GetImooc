...
var h_categories
var foot = ">数据来源于[慕课网](http://www.imooc.com)，由`程序员的萌工厂`整理发布，QQ群：300421872"


fs.exists('Github', function(exists) {
	if (exists)
		console.log('Github文件夹存在');
	else {
		fs.mkdir('Github', function(err) {
			if (err)
				console.error(err);
			console.log('创建Github文件夹成功');
		})
	}
...
function touchMD(fn) {
	...
	fs.writeFile('./Github/' + fn + '.md', "", function(err) {
		if (err) throw err;
		console.log("文档名：" + fn);
	})
	...
	toc = hexo_toc + '\n\n'
	hexo = '>' + h_date + h_tags + course_count + toc;

	// fs.appendFileSync('./Github/' + fn + '.md', hexo + course_count + toc, function(err) {
	// 	if (err) throw err;
	// });
	...
	// 追加 MD的title
	fs.appendFile('./Github/' + fn + '.md', Mtitle + hexo, function(err) {
		if (err) throw err;
	});
	...
	for (let i = 0; i <= readall.course.length - 1; i++) {
		...
		fs.appendFile('./Github/' + fn + '.md', Mlevel_title + Mlevel_info, function(err) {
			if (err) throw err;
		});
		...
		for (var j = 0; j <= courseI.course_list.length - 1; j++) {
			...
			fs.appendFile('./Github/' + fn + '.md', Mcourse_list0 + Mcourse_list2, function(err) {
				if (err) throw err;
			});
		}
	}
	...
	glob(Mfiles, 'utf-8', function(err, mfiles) {
		if (err) {
			...
			for (let l = 0; l <= mfiles.length - 1; l++) {
				mfiles[l] = mfiles[l].replace('.md', '').replace('Github/', '');
				fs.appendFile('./Github/' + mfiles[l] + '.md', foot, function(err) {
					if (err) throw err;
					if (l == mfiles.length - 1) {
						console.log('全部文件数量为：' + mfiles.length + "，保存在Github文件夹中！")
					}
				});
			}
		}
